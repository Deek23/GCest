...
#pragma omp parallel for if(!useCUDA)
		for (int targetCol = 0; targetCol < kNumObservations; targetCol++)
		{
			...
			for (int i = data.init - 20; i <= data.end - 20; ++i)
			{
				...
				ceres::CostFunction *costFunc = new ceres::AutoDiffCostFunction<MitscherlichResidual, 1, n_parameter>(data.age(i), data.height(i));
				...
				// パラメータ範囲指定
      // A
      problem.SetParameterLowerBound(p, 0, 0.0);
      problem.SetParameterUpperBound(p, 0, 60.0);
      // L
      problem.SetParameterLowerBound(p, 1, 1.0);
      problem.SetParameterUpperBound(p, 1, 7.2);
      // K
      problem.SetParameterLowerBound(p, 2, 0.0);
      problem.SetParameterUpperBound(p, 2, 0.1);
			};
