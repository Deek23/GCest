...
#pragma omp parallel for if(!useCUDA)
		for (int targetCol = 0; targetCol < kNumObservations; targetCol++)
		{
			...
			for (int i = data.init - 20; i <= data.end - 20; ++i)
			{
				...
				ceres::CostFunction *costFunc = new ceres::AutoDiffCostFunction<RichardsResidual, 1, n_parameter>(data.age(i), data.height(i));
      ...
      // k
      problem.SetParameterLowerBound(p, 1, 0.01);
      problem.SetParameterUpperBound(p, 1, 0.2);
      // m
      problem.SetParameterLowerBound(p, 2, 0.01);
      problem.SetParameterUpperBound(p, 2, 0.99);
			};
