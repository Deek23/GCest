...
#pragma omp parallel for if(!useCUDA)
		for (int targetCol = 0; targetCol < kNumObservations; targetCol++)
		{
			...
			for (int i = data.init - 20; i <= data.end - 20; ++i)
			{
				...
				ceres::CostFunction *costFunc = new ceres::AutoDiffCostFunction<GompertzResidual, 1, n_parameter>(data.age(i), data.height(i));
				...
				// パラメータ範囲指定
      // K
      problem.SetParameterLowerBound(p, 0, 10.0);
      problem.SetParameterUpperBound(p, 0, 80.0);
      // b
      problem.SetParameterLowerBound(p, 1, 1.0);
      problem.SetParameterUpperBound(p, 1, 16.5);
      // c
      problem.SetParameterLowerBound(p, 2, 0.01);
      problem.SetParameterUpperBound(p, 2, 0.14);
			};
